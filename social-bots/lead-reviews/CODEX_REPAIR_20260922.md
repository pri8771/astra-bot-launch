# Social Bots host-test repair — Codex implementation, lead review requested

State: **READY_FOR_LEAD_REVIEW / NO_FABLE_HANDOFF**. The owner now directs Codex to finish directly. Codex authored this isolated test-only repair and cannot independently accept it. ChatGPT/Mac Acceptance retain formal/independent review. No Fable dispatch is authorized or requested.
...
- Local repair: `codex/bots-host-tests-20260922@7acc1695d2961267580a156f31df6a5991476654`; clean separate worktree; published in [code PR #6](https://github.com/pri8771/astra-bot-launch/pull/6), targeting the actual worker base.
...
Next bounded assignment: **Codex, SP1 review repair** if an independent reviewer finds a concrete defect; otherwise retain the exact candidate for lead verdict. Request the missing LEAD-052 detailed record from its actual author. Fresh owner-designated persistent-host/scheduler/zero-charge-route/reviewer context is pending for the genuine V0.4–V0.7 sequence; no authorization manifest or account registry was found. Local preflight is INCONCLUSIVE_NEED_OWNER_CONFIRMATION, not host qualification. No new scheduler, model call, public action or grant is part of this packet. SESSION_ONCE remains once per fresh session; no extra emission on continuation or compaction. No Fable handoff.

## Authorized exact-SHA verdict observed

Canonical `290eaefab581901c9643e7de8719829150c59acf` now contains ChatGPT's `LEAD-053_2026-09-22T0344.md`: **SB-R07-072 ACCEPTED at7acc1695d2961267580a156f31df6a5991476654, test-only**. The lead independently inspected the exact diff and source-bound author evidence; it did not claim an independent execution. Real host/V0.7 acceptance remains ungranted. The actual LEAD-052 detailed record is now present at canonical, resolving the missing-record finding. No extra copy or reconstructed verdict was authored here.

The canonical queue preserves existing unrelated ownership and explicitly directs new concrete repairs to isolated Codex branches, with no Fable handoff. This observed lead verdict supersedes the repair REVIEW_BLOCKED statement above only for SB-R07-072 at that exact SHA; all genuine product-path gates remain.
